  ...
  describe('coreGetEpicBootstrap', () => {
    it('should bootstrap the app', done => {
      ...
      coreGetEpicBootstrap(ActionsObservable.of(actions.bootstrap(token)), state$, deps).subscribe(output => {
        ...
        if (output.type === productsState.ActionType.SET_LIST_START) {
          ...
          expect(emitedActions[1]).toEqual(productsState.actions.setListStart({ page: 1, limit: ENV.PAGINATION.LIMIT }));
          done();
        }
      });
    });
