...
export const coreGetEpicBootstrap: Epic<IAction, IAction, IRootState, IEpicDependencies> = (action$, state$, deps) =>
  action$.pipe(
    ...
    /** merge maps emits get current user & first products list fetch data and isolates an observable branch in case it fails  */
    mergeMap(() =>
      of(userActions.setListStart([state$.value.auth.currentUserId]), productsActions.setListStart({ page: 1, limit: ENV.PAGINATION.LIMIT })).pipe(
        /** side effect to navigate to initial authenticated view and reset the router so the user can't go back to the login */
        tap(() => {
          const resetAction = StackActions.reset({ index: 0, actions: [NavigationActions.navigate({ routeName: 'ProductsList' })] });
